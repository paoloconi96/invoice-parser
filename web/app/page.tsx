...
export default function Home() {
    ...
    const updateFiles = (files: Invoice[]) => {
        setFiles(files);
        setFileTableBody(files.map(file => {
            return <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={file.id}>
    ...
    return (
        <main className="flex min-h-screen flex-col items-center p-24">
            <form onSubmit={handleSubmit}>
                <label htmlFor="file" className="block text-sm font-medium leading-6">File *</label>
                <input type="file" name="file" value={inputValue} onChange={handleInputValueChange}/>
